...
- **`AAOSP` (this commit)** — README + ROADMAP positioning pass after surveying Android 16's `androidx.appfunctions`. README gains a "Why AAOSP, not just App Functions?" section (architectural inversion: cloud-Gemini-orchestrated vs on-device-LLM + MCP-wire-format + HITL/audit built into the runtime) and a "Same playing field for OEMs" subsection (AAOSP gives any OEM the AICore-tier capability without renting the runtime from Google — no Gemini license, no telemetry, OEM controls model + consent UX + audit). ROADMAP gains an "Android App Functions adapter" exploratory entry — auto-expose registered MCP tools as `@AppFunction`s on Android 16+ devices so MCP-app authors get both surfaces from one manifest block; gated on rebasing the framework fork to Android 16.
