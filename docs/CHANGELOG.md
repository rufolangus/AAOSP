...
- **`AAOSP` (this pass)** — Framing reset across README + ROADMAP + DESIGN_NOTES. Prior documentation positioned AAOSP as *"the open AICore"* and framed App Functions as a competitor to respond to; both framings are retired. AAOSP is a reference / research project that brought MCP + on-device LLM + platform-level HITL + audit to Android 15 at a point when stock Android had no cross-app agent substrate. Android 16 later shipped App Functions (`@AppFunction` + `AppFunctionManager`) as its own cross-app tool-invocation layer — convergent evidence that the primitive belongs at the platform tier. AAOSP stays on Android 15 as the reference artifact; it does not compete with App Functions and does not migrate to Android 16. The `android16-migration` hypothesis branch was deleted from the umbrella remote on 2026-04-16. Concrete changes: **README** — new tagline *"MCP + on-device LLM on Android 15 — built before Android 16 shipped its own MCP-equivalent"*; the entire *"How AAOSP relates to App Functions, AICore, and Google Assistant"* section (layer tables, comparison table, regulatory paragraph, OEM subsection) replaced with a short *"Where this fits"* section + *"What's Apache 2.0 in AAOSP"* bullet list; *"What This Is"* drops the *"AAOSP is to AICore what AOSP is to GMS"* line and is restructured around three platform-tier contributions + AgenticLauncher; *"What AAOSP won't do"* drops the AICore-swap comparator; *"Who this is for"* reframed around platform folks, app developers, on-device-AI researchers. **ROADMAP** — *"Android App Functions adapter"* entry deleted (was gated on rebasing the framework fork to Android 16; that rebase is not happening). **DESIGN_NOTES** — orchestrator-pluggability section's AICore + Gemini Nano + Google Assistant comparator sentence removed; the "pick your model, pick your assistant" argument stands on its own.
